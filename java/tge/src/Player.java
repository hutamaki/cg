...
class OptimizedBoard {
	...
	public boolean isWallPossible(int x, int y, char orientation, Dragoon[] dragoons, HashMap<Integer, Astarable<Unit>> pathes) {
		...
		for (Entry<Integer, Astarable<Unit>> entry : pathes.entrySet()) {
			Node<Unit> way = dfs.search(entry.getValue(), dragoons[entry.getKey()], nb_total);
			if (way == null) {
				isWallPlacable = false;
				break;
			}
		}
...
class Game {
	...
	public void guruMeditation() {
		...
		if ((id != myId) && (getMe().wallsLeft > 0)) { // find
			...
			if (best_placement != null && tyx < his_x) {
